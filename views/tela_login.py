...
class TelaLogin:
    ...
    def entrar(self, e):
        login_temp = Cliente(nome="", email=self.email.value, senha=self.senha.value)
        usuario_autenticado = self.login_controle.autenticar(login_temp)
        
        if isinstance(usuario_autenticado, Cliente):
            self.page.clean()
            PainelCliente(self.page, usuario_autenticado)
        elif isinstance(usuario_autenticado, Admin):
            self.page.clean()
            PainelAdmin(self.page)
        else:
            self.msg_text.value = usuario_autenticado
